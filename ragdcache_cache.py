...
class KVCacheManager:
    def __init__(self, kvcache_dir, cache_size_gb=16, llm_model="facebook/opt-1.3b"):
        """
        Initializes the KVCacheManager.

        Args:
            kvcache_dir (str): Directory where cache files are stored on disk.
            cache_size_gb (int, optional): Maximum cache size in gigabytes. Defaults to 16.
            llm_model (str, optional): The language model to use. Defaults to "facebook/opt-1.3b".
        """
        self.kvcache_dir = kvcache_dir
        self.cache = OrderedDict()  # OrderedDict to maintain LRU order
        self.cache_size_limit = cache_size_gb * (1024 ** 3)  # Convert GB to bytes
        self.current_cache_size = 0  # Current cache size in bytes
        self.model = llm_model

    def _get_size(self, item):
        """
        Calculates the memory size of a cached item in bytes.

        Args:
            item: The cached item (e.g., a PyTorch tensor).
        ...
        if isinstance(item, torch.Tensor):
            ...
            # If the item is a dictionary, sum the sizes of all values
            return sum(self._get_size(v) for v in item.values())
        elif isinstance(item, list) or isinstance(item, tuple):
            # If the item is a list or tuple, sum the sizes of all elements
            return sum(self._get_size(v) for v in item)
        else:
            # For other types, estimate size using sys.getsizeof
            import sys
            return sys.getsizeof(item)

    def load_kvcache(self, idx):
        """
        Loads the cache entry for the given index. If the entry is in memory,
        it returns immediately. Otherwise, it loads from disk and manages the cache.

        Args:
            idx (str or int): The key/index of the cache entry.

        Returns:
            tuple: (cached_data, cache_type)
                   cached_data: The cached data.
                   cache_type: 1 if loaded from memory, 0 if loaded from disk.
        """
        cache_type = 1  # Assume from Memory

        if idx in self.cache:
            # Move the accessed item to the end to mark it as recently used
            self.cache.move_to_end(idx)
            return self.cache[idx][0], cache_type
        else:
            kvcache_path = os.path.join(self.kvcache_dir, f'kvcache_{idx}.pt')
            if os.path.exists(kvcache_path):
                # Load from disk
                cached_data = torch.load(kvcache_path)
                item_size = self._get_size(cached_data)

                # Evict items if necessary
                self._evict_if_needed(item_size)

                # Add to cache
                self.cache[idx] = (cached_data, item_size)
                self.current_cache_size += item_size
                cache_type = 0  # Loaded from disk
                return cached_data, cache_type
            else:
                # Cache miss; handle accordingly (e.g., return None or raise an error)
                return None, -1  # -1 indicates not found

    def _evict_if_needed(self, item_size):
        """
        Evicts least recently used items until there is enough space for the new item.

        Args:
            item_size (int): Size of the new item to be added in bytes.
        """
        while self.current_cache_size + item_size > self.cache_size_limit and self.cache:
            # Evict the least recently used item (first item in OrderedDict)
            evicted_idx, (evicted_data, evicted_size) = self.cache.popitem(last=False)
            self.current_cache_size -= evicted_size
            # Optionally, you can save the evicted item back to disk if needed
            # torch.save(evicted_data, os.path.join(self.kvcache_dir, f'kvcache_{evicted_idx}.pt'))

    def clear_cache(self):
        """
        Clears the entire in-memory cache and performs garbage collection.
        """
        self.cache.clear()
        self.current_cache_size = 0
        gc.collect()

    def make_kvcache(self, idx, context):
        """
        Make KV cache for new context. 

        Args:
            idx (str or int): The key/index for the context
            context : data to be KV cached
        """
        kvcache = context
        item_size = self._get_size(kvcache)
        self._evict_if_needed(item_size)
        self.cache[idx] = (kvcache, item_size)
        self.current_cache_size += item_size

    def save_all_to_disk(self):
        """
        Saves all in-memory cache entries to disk.
        """
        for idx, (data, _) in self.cache.items():
            kvcache_path = os.path.join(self.kvcache_dir, f'kvcache_{idx}.pt')
            torch.save(data, kvcache_path)
...
def run_improved_rag(model_size):
    ...
    for bsize in range(1, 11):
        results = []
        kvcache_manager.clear_cache() # KV memory Cache 초기화
        ...
        with torch.no_grad():
            ...
            for batch_idx, (batch_prompts, batch_kv_caches, batch_kv_attention_masks, batch_past_seq_lens, batch_cache_types, batch_full_prompts, batch_search_time, batch_kv_load_time) in pbar:
                ...
                pbar.set_postfix({
                    'Fast' : fast_t,
                    '[KV] PF': f'{prefill_time_kv:.4f}',
                    'Load(from)': f'{batch_kv_load_time:.4f}{batch_cache_types}',
                    '[NK] PF': f'{prefill_time:.4f}',
                })
        
        ######################## 결과를 파일로 저장 ########################
        csv_file = f'./result/withKVRAG_cache_result_all.csv'
